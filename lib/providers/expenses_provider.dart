...
class ExpensesState extends ChangeNotifier {
  ...
  /// Update an expense header and replace its items atomically.
  Future<void> updateExpenseWithItems(Expense expense, List<ExpenseItemsCompanion> items) async {
    ...
    await dao.transaction(() async {
      ...
      await (dao.delete(dao.expenseItems)..where((t) => t.expenseId.equals(expense.id))).go();

      // Insert new items, wiring the expenseId
      for (final it in items) {
        // copyWith isn't available on companions generated by Drift for insert constructors,
        // but we can ensure expenseId is set here by creating a new companion
        ...
        await dao.into(dao.expenseItems).insert(companion);
      }
    });
  }
}
